...
export async function sendGempaDirasakanNotification(
  ...
  try {
    ...
    console.log(`[FCM] Total tokens dari DB: ${tokens.length}`);
    console.log(`[FCM] Setelah deduplikasi: ${uniqueTokens.length}`);
    console.log(`[FCM] Duplikat dihapus: ${duplicateCount}`);
    console.log(
      `[FCM] Tokens:`,
      uniqueTokens.map((t) => t.slice(0, 20) + "..."),
    );
