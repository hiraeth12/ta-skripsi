...
async function syncFullGempaDirasakan() {
  ...
  if (!currentRes.ok) {
    throw new Error(
      `Failed to read current DB node: ${currentRes.status} ${currentRes.statusText}`,
    );
  }
  ...
  const sortedItems = sortItemsAscendingByDate(mergedItems);
  ...
  const payload = {
    ...
    lastEventId: lastItem?.eventid || null,
    items: sortedItems,
  };
  ...
  return {
    ...
    apiTotalItems: normalizedItems.length,
    currentTotalItems: currentItems.length,
    mergedTotalItems: sortedItems.length,
    addedItems: sortedItems.length - currentItems.length,
    lastEventId: lastItem?.eventid,
    sourceUrl: apiUrl,
  };
}
